...
class MyEnvironment(ccm.Model):
    pass
class VModule(ccm.ProductionSystem):
    production_time=0.02
    
    def Dream(DM='busy:False'):
        vision_module.request('')
        print "day dreaming"
        
class MyEmotionModule(ccm.ProductionSystem):
    production_time=0.01
    
    def caution(focus='sandwich ham'):
        print "Oh no!!!"
        Ebuffer.set('stressed')
