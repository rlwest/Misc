...
class MyAgent(ACTR):
    ...
    def init():                                             
        ...
        DM.add ('task_goal:find_bread constraint:bread_on_plate')
...
############ this would be used for a planning unit that must be done immediatly if another fails
    ...
    def recovery_procedure(Focusbuffer='unit_task:?unit_task task_goal:?task_goal',
                           Contextbuffer='status:problem planning_unit:hamcheese_sandwich',
                           utility=1.5):
...
# this fires if a unit task production fails to fire
    def abort_planning_unit(Focusbuffer='unit_task:?unit_task task_goal:?task_goal',
                            ...
                            Contextbuffer='status:ok planning_unit:?planning_unit',
                            utility=-0.5):
        print 'aborted unit task= ',unit_task,'task goal= ',task_goal     
        Focusbuffer.set('unit_task:?unit_task task_goal:start')#### can restart task goal
        Contextbuffer.set('status:problem planning_unit:?planning_unit')#### can opt out of planning unit
...
# this stops looping based on a repeating problem
    def flag1(Focusbuffer='unit_task:?unit_task task_goal:?task_goal',
                    DMbuffer='state:fail planning_unit:?planning_unit unit_task:?unit_task task_goal:?task_goal',
                    Contextbuffer='status:problem planning_unit:?planning_unit'):
        print 'I remember this'
        print 'state:fail',' planning_unit:',planning_unit,' unit_task:',unit_task,' task_goal:',task_goal
        DM.request('task_goal:?task_goal constraint:?constaint')
        Focusbuffer.set('state:solve unit_task:?unit_task task_goal:?task_goal')
        Contextbuffer.set('status:solving planning_unit:?planning_unit')
    def flag2(Focusbuffer='unit_task:?unit_task task_goal:?task_goal',
                    DMbuffer='task_goal:?task_goal constraint:?constraint',
                    Contextbuffer='status:solving planning_unit:?planning_unit'):
        print 'Constraint'
        print constraint
        DM.request('task_goal:?task_goal world:?world')
        Focusbuffer.set('constraint:?constraint unit_task:?unit_task task_goal:?task_goal')
        DM.add ('task_goal:find_bread constraint:bread_on_plate')
    def flag_ok(Focusbuffer='constraint:?constraint unit_task:?unit_task task_goal:?task_goal',
                DMbuffer='task_goal:?task_goal constraint:?constraint',
                Contextbuffer='status:solving planning_unit:?planning_unit'):
        print 'I remember constraint is ok in the world'
        Focusbuffer.set('stop')
    def flag_notok(Focusbuffer='constraint:?constraint unit_task:?unit_task task_goal:?task_goal',
                   DMbuffer='task_goal:?task_goal world:?constraint',
                   Contextbuffer='status:solving planning_unit:?planning_unit'):
        print 'I remember constraint is not ok in the world'
        Focusbuffer.set('stop')
...
# this is the first operator in each unit task
# requests a dm chunk on this unit task - - could be done by a bottom up dm production
    def start(Focusbuffer='unit_task:?unit_task task_goal:start'):
        print 'start',unit_task
        DM.request('state:?state planning_unit:?planning_unit unit_task:?unit_task task_goal:?task_goal')
        Focusbuffer.set('unit_task:?unit_task task_goal:go')
...
## bread bottom
    ...
    def bread_find(Focusbuffer='unit_task:bread_bottom task_goal:go',
    ...
    def bread_moved(Focusbuffer='unit_task:bread_bottom task_goal:move_finished',
        ...
        DM.add('unit_task:bread_bottom location:on_plate')
