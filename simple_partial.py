...
class MyAgent(ACTR):
    ...
    def init():
        DM.add('customer1 mustard')         # customer1's order
        DM.add('customer2 ketchup')         # customer2's order
        DM.add('customer3 mayonnaise')      # customer3's order
    ...
    def condiment(focus='customer1 condiment'):  # customer1 will spread activation to 'customer1 mustard'
        ...
        DM.request('customer1 ?')               
        focus.set('sandwich condiment') 

    def order(focus='sandwich condiment', DMbuffer='? ?condiment'):  
