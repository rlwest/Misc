...
############ this would be used for a planning unit that must be done immediatly if another fails
    ...
    def cannot_find(Focusbuffer='state:fail unit_task:?unit_task task_goal:?task_goal',
                    Contextbuffer='status:problem planning_unit:hamcheese'):
        Contextbuffer.set('status:ok planning_unit:ham_sandwich') # new planning unit
        Focusbuffer.set('cue:start') #start at the begining
...
## bread bottom 
    ...
    def bread_start(Focusbuffer='unit_task:bread_bottom task_goal:start'):
        print 'requesting information about bread'
        DM.request('unit_task:bread location:?location')
        Focusbuffer.set('unit_task:bread_bottom task_goal:get_info')
        Motorbuffer.set('state:waiting')

    def bread_start2(Focusbuffer='unit_task:bread_bottom task_goal:get_info',
                     DMbuffer='unit_task:bread location:?location'):
        print 'getting the information about bread'
        Focusbuffer.set('unit_task:bread_bottom task_goal:find_bread')
        Visualbuffer.set('location:?location')
    ...
    def bread_find(Focusbuffer='unit_task:bread_bottom task_goal:find_bread',
                   ...
                   Visualbuffer='location:on_counter'): 
        print 'bread is in start place'
        Focusbuffer.set('unit_task:bread_bottom task_goal:move_bread')
        Motorbuffer.set('state:waiting')

    def bread_move(Focusbuffer='unit_task:bread_bottom task_goal:move_bread'): 
        print 'moving bread'
        Motor.do_bread()
        Focusbuffer.set('unit_task:bread_bottom task_goal:find_bread')
        Motorbuffer.set('state:not_waiting')

    def bread_moved(Focusbuffer='unit_task:bread_bottom task_goal:find_bread',
                    bread='location:on_plate',
                    Visualbuffer='location:on_plate'): 
