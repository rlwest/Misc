...
class MyAgent(ACTR):
    ...
    def init():                                             
        ...
        DM.add ('step:bread_bottom state:done')
...
# unit task cycle productions
    ...
    def retrieve_unit_task(Focusbuffer='state:unit_task',
                           ...
                           DM='busy:False'):
        DM.request('step:?unit_task state:?state') # check for info - used below
...
# requests a dm chunk on this unit task - - could be done by a bottom up dm production
    def done(Focusbuffer='unit_task:?unit_task task_goal:start',
              DMbuffer='step:?unit_task state:done'):
        print 'done, move on...'
        print 'skip',unit_task
        Focusbuffer.set('unit_task:bread_bottom task_goal:complete')
    def not_done(Focusbuffer='unit_task:?unit_task task_goal:start',
              DMbuffer='step:?unit_task state:not_done'):
        print 'not done yet'
...
## bread bottom
    ...
    def bread_find(Focusbuffer='unit_task:bread_bottom task_goal:go',
        ...
        Focusbuffer.set('unit_task:bread_bottom task_goal:finished')
        Motorbuffer.set('state:waiting')########
        Motor.do_bread()

    def bread_moved(Focusbuffer='unit_task:bread_bottom task_goal:finished',
                    bread='location:on_plate'): 
        print 'bread is in end place'
        Focusbuffer.set('unit_task:bread_bottom task_goal:complete')
        Motorbuffer.set('state:not_waiting')##############
        DM.add ('step:bread_bottom state:done')########## DM.add
